...
export type BulkAssetCategoriesUploadDialogProps = {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  onUploadSuccess?: () => void;
};

export function BulkAssetCategoriesUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkAssetCategoriesUploadDialogProps) {
  ...
  async function handleDownloadTemplate() {
    setDownloading(true);
    try {
      const response = await fetch('/api/asset-categories/template');
      if (!response.ok) throw new Error('Failed to download template');
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset_categories_template.xlsx';
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch('/api/asset-categories/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload asset categories';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} record(s)`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload asset categories';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Upload Asset Categories</DialogTitle>
          <DialogDescription>
            Download the Excel template, fill in the asset categories, and upload it back.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">The template contains the required fields: Category and Asset Group Name.</p>
          </div>
