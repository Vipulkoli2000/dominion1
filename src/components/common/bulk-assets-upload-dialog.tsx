...
export type BulkAssetsUploadDialogProps = {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  onUploadSuccess?: () => void;
};

export function BulkAssetsUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkAssetsUploadDialogProps) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload assets';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} asset record(s)`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload assets';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Download the Excel template, fill in the assets, and upload it back.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">Template requires Asset Group Name, Asset Category, and Asset Name.</p>
          </div>
