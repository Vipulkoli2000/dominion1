...
export type BulkStatesUploadDialogProps = {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  onUploadSuccess?: () => void;
};

export function BulkStatesUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkStatesUploadDialogProps) {
  ...
  async function handleDownloadTemplate() {
    setDownloading(true);
    try {
      const response = await fetch('/api/states/template');
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch('/api/states/upload', { method: 'POST', body: formData });
      const data = await response.json().catch(() => null);
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload states data';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} state records`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload states data';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Upload States</DialogTitle>
          <DialogDescription>
            Download the Excel template, fill in the states, and upload it back.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">The template contains the mandatory State field.</p>
          </div>
